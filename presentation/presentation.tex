...
\begin{frame}{Problem \& Dataset}
  \begin{columns}[T]
    ...
    \column{0.40\textwidth}
      \centering
      \includegraphics[width=\linewidth]{detection_samples.png}\\
...
\begin{frame}{Training Recipe}
  ...
  \begin{block}{\small Optimiser note}
    ...
    We add \textbf{FCOS (SGD)} as \emph{our own} ablation. Total cost: $\sim$3.5\,h, $\sim$\$30--40.
  \end{block}
\end{frame}
...
\begin{frame}{The Calibration Artefact (our strongest result)}
  \small
  At fixed $\tau=0.3$, the WBF ensemble ``wins'' patient F1 ($63.0\%$, $+9.8$ over the best single).
...
\begin{frame}{Robustness Checks (no extra training)}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \centering
      \includegraphics[width=\linewidth]{ap_bootstrap.png}
  \end{columns}
\end{frame}
...
\begin{frame}{Conclusion}
  ...
  \begin{enumerate}\setlength\itemsep{0.45em}
    ...
    \item We do \emph{not} reproduce the paper's absolute numbers; the gap is one of
      \hl{evaluation} (recall $\sim$96--98 vs.\ 85), not training.
