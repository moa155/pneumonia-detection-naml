...
BF16 gave $\sim$1.8$\times$ training throughput over FP32 in preliminary runs and, by preserving FP32's exponent range, eliminated the occasional NaN/Inf we saw with FP16 on RetinaNet's focal-loss branch and the Faster R-CNN RPN, so the four-model parallel run completes in $\approx 2.5$ wall-clock hours ($\sim$\$30 total).
