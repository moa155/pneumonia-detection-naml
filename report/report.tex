...
\begin{abstract}
This report reproduces and extends the method proposed by Wu et al.~\cite{wu2024pneumonia}, which applies the anchor-free FCOS (Fully Convolutional One-Stage) detector to pneumonia detection on chest X-rays from the RSNA Pneumonia Detection Challenge dataset. We implement and compare four detector variants sharing the same ResNet-50 + Feature Pyramid Network (FPN) backbone: (1)~FCOS with the paper's anchor-free head trained under our modern Adam recipe, (2)~the same FCOS head retrained under the paper's original SGD+momentum recipe as an optimiser ablation, (3)~RetinaNet as a one-stage anchor-based baseline, and (4)~Faster R-CNN as a two-stage anchor-based baseline. Training is performed on the full RSNA dataset (26{,}684 images) for 40 epochs on four NVIDIA H100 80\,GB SXM GPUs (one model per GPU) with BF16 mixed-precision, cosine annealing, warm-up, and Exponential Moving Average (EMA) of the weights. At evaluation time we apply horizontal-flip Test-Time Augmentation (TTA) and Gaussian Soft-NMS, and combine FCOS with RetinaNet via Weighted Box Fusion (WBF).

All detectors \emph{exceed} the paper's reported FCOS AP@0.5 of $28.5\%$: our FCOS (Adam) reaches $35.3\%$ ($+6.8$), FCOS with the paper's SGD recipe reaches $39.8\%$ ($+11.3$), RetinaNet reaches $41.2\%$ ($+12.7$), and Faster R-CNN reaches $42.9\%$ ($+14.4$). The central empirical finding is that, under a shared backbone and a shared \emph{Adam} recipe, the anchor-based detectors decisively outperform the anchor-free FCOS on box-level AP@0.5; switching FCOS to the paper's SGD recipe narrows but does \emph{not} close the gap to RetinaNet (Section~\ref{sec:robustness} reports the paired bootstrap on the per-patient difference). Crucially, this is an \emph{asymmetric} optimiser ablation --- we only re-trained FCOS under SGD --- so the result tightens the FCOS-vs-RetinaNet comparison without resolving the underlying paradigm-vs-optimiser confound. At the patient level, the WBF ensemble reaches F1 $63.0\%$ at the fixed threshold $\tau = 0.3$ used by the paper, which is $+13.5$ points above the best single model \emph{at that arbitrary cut}; this headline is largely a calibration artefact, as Section~\ref{sec:robustness} demonstrates by refitting a Youden-optimal threshold on a held-out calibration half and by training a $5$-fold-CV logistic aggregator on per-patient features. Under either fair protocol the ensemble's lead shrinks to $\le 1$ F1 point and is overtaken by RetinaNet (learnt aggregator F1 $67.3\%$, the single best patient-level number in the study). All point estimates carry two caveats that the bootstrap does not address: they come from a single training seed, so run-to-run noise is uncovered; and they are read at each detector's best-validation EMA checkpoint, on the same split the bootstrap CIs are computed on. We discuss both in Section~\ref{sec:limitations}.
\end{abstract}
...
\begin{itemize}[nosep]
    \item A re-implementation of Wu et al.~\cite{wu2024pneumonia} on the full RSNA dataset (we do \emph{not} claim a bit-for-bit reproduction: the paper releases neither code nor weights, and we modernise the recipe with BF16, EMA, cosine annealing and medical-specific albumentations that the paper does not use). Our FCOS-Adam crosses the paper's reported $28.5\%$ AP@0.5 at epoch~$12$ and ends $+6.8$ points above it at the $40$-epoch convergence; the SGD-recipe ablation reaches $+11.3$ points above the paper number.
    \item A same-backbone, same-budget comparison of three representative detection paradigms (anchor-free FCOS, one-stage anchor-based RetinaNet, two-stage anchor-based Faster R-CNN) under a shared \emph{Adam} recipe, plus an FCOS-only SGD ablation that replicates the optimiser of Wu et al. The Adam-trained FCOS is decisively below the anchor-based detectors on AP@0.5; under the paper SGD recipe the FCOS-vs.-RetinaNet gap narrows. We have \emph{not} re-trained RetinaNet or Faster R-CNN under SGD, so the comparison separates paradigm from optimiser only on the FCOS side.
    \item A Weighted Box Fusion ensemble of the two one-stage detectors (FCOS + RetinaNet) that improves patient-level F1 at the fixed $\tau=0.3$ cut without retraining; under a fair per-detector operating point (Section~\ref{sec:robustness}) the ensemble matches but does \emph{not} dominate the best single detector.
    ...
    \item A public PyTorch implementation with the training recipes (cosine + warm-up schedule, EMA, medical-imaging augmentation pipeline) and all evaluation artefacts released with the report. Only the patient-level split (seed $42$) is bit-deterministic; the training run itself is not, because TF32 + BF16 + asynchronous dataloader workers + nondeterministic CUDA atomics break exact reproducibility.
\end{itemize}
...
All detectors surpass Wu et al.'s reported FCOS AP@0.5 of $28.5\%$. Faster R-CNN achieves the best single-model AP@0.5 at $42.9\%$ ($+14.4$). Our FCOS (Adam) reproduces the paper's AP trajectory and ends $+6.8$ above the paper's reported score; switching to the paper's SGD+momentum optimiser (the ``FCOS (paper SGD)'' row in Table~\ref{tab:comparison}) lifts FCOS by $+4.5$ to $39.8\%$, narrowing but not closing the gap to RetinaNet ($41.2\%$, paired-bootstrap test in Section~\ref{sec:robustness}). The FCOS+RetinaNet WBF ensemble matches Faster R-CNN on AP@0.5 ($42.0$ vs $42.9$) and trails by a single point on FROC CPM (cf.\ Section~\ref{sec:robustness}). The size-stratified $AP_S^{\mathrm{R}}$, $AP_M^{\mathrm{R}}$, $AP_L^{\mathrm{R}}$ use the tertiles of the actual RSNA opacity-area distribution rather than the COCO $32^2 / 96^2$ cutoffs; the latter would put $\sim 99\%$ of RSNA boxes into ``large'' and produce $AP_M \equiv 0$ for every detector. Reading across the row: AP grows monotonically with opacity size (small opacities are intrinsically harder); reading down a column: the FCOS-vs.-RetinaNet gap is small on the small bucket (where the absolute AP is too low and the count too small to discriminate detectors anyway), modest on medium, and widest on large opacities. $AR_{10}$ is high ($83$--$87\%$) across the board: when all models are allowed $10$ detections per image they recover most ground truths at IoU $\ge 0.5$. Each entry in Table~\ref{tab:comparison} is the best-validation EMA checkpoint of a single training run, evaluated on the same validation split used for checkpoint selection; this introduces an upward selection bias (the AP is a max over $\sim\!10$ noisy validation estimates per detector) and the variance is single-seed only --- caveats we revisit in Sections~\ref{sec:robustness} and~\ref{sec:limitations}.

\paragraph{FCOS paper-recipe ablation: SGD narrows but does not close the gap.}
The FCOS (paper SGD) row in Table~\ref{tab:comparison} substitutes our Adam configuration with the SGD+momentum recipe described by Wu et al.\ (momentum $0.9$, weight decay $1.6\times10^{-4}$, base learning rate $5\times10^{-3}$, $40$ epochs with $2$-epoch warm-up). Under this recipe, FCOS validation AP@0.5 rises through epoch $24$ and plateaus thereafter (best $39.9\%$ at epoch $24$, end-of-training $39.3\%$ at epoch $40$; cf.\ Figure~\ref{fig:val_ap}). The TTA+Soft-NMS evaluation lifts the test-time score to $39.8\%$. Two notes of caution are in order. First, the overlap between the FCOS-SGD $95\%$ CI $[37.1, 43.1]$ and RetinaNet's $[38.2, 44.1]$ is \emph{not} by itself evidence of equivalence: the correct test is a paired bootstrap on the difference (Section~\ref{sec:robustness}), and even there ``failure to reject'' should not be confused with ``equivalence''. Second, we only re-ran FCOS under SGD --- not RetinaNet, not Faster R-CNN --- so any narrative that ``the recipe explains the gap'' is supported only on the FCOS side; we cannot rule out that the anchor-based detectors would also gain under SGD. The honest framing is therefore: under the paper's SGD recipe FCOS narrows the gap to RetinaNet, the per-patient paired test is what decides whether the residual difference is signal or noise (Section~\ref{sec:robustness}), and the paradigm gap to Faster R-CNN ($42.9\%$) shrinks but does not vanish.
...
On the positive cases (rows 1--3) every detector produces boxes, but the spatial agreement with the ground-truth boxes varies. The two anchor-based detectors (RetinaNet, Faster R-CNN) consistently produce boxes that closely cover the ground-truth opacity; FCOS and FCOS (paper SGD) tend to over-estimate the lateral extent of the opacity, especially on row~2 where the ground-truth boxes are tall and narrow. The mechanism here is the FCOS \emph{regression head}, which predicts the four edge distances $(l, t, r, b)$ directly from each spatial location with no aspect-ratio prior --- so a wide opacity and a tall narrow one can both be hit by the same regression branch, with no shape inductive bias to penalise an over-wide box. (The centre-ness branch is sometimes blamed for this; it should not be. Centre-ness predicts a single $[0,1]$ score that re-weights classification confidence to down-weight off-centre points; it does \emph{not} regress box geometry.) Anchor-based detectors, by contrast, sample $A$ anchors per location whose shapes encode an explicit aspect-ratio prior, and the regression target is an offset from that anchor, which biases the output towards anchor-shaped boxes.
...
We resample the validation patients with replacement $B = 500$ times and recompute AP@0.5 on each resample. The resulting $95\%$ percentile intervals (Figure~\ref{fig:ap_bootstrap} and Table~\ref{tab:robust_detection}) quantify the within-split variance of each detector's AP@0.5. The intervals are computed independently per detector, so they capture only the variance of resampling validation patients on a single trained model; they do \emph{not} estimate run-to-run variance from training-seed changes (initialisation, augmentation RNG, optimiser stochasticity), which in detection is routinely $\pm 1$--$2$ AP and which is therefore an additional, uncovered source of noise on top of every interval below. The interval of FCOS Adam $[32.6, 38.4]$ does not overlap with that of Faster R-CNN $[40.0, 46.3]$, so the Adam-FCOS-vs-Faster-R-CNN ordering is robust to patient resampling at the $95\%$ confidence level. The FCOS (paper SGD) interval $[37.1, 43.1]$ overlaps RetinaNet's $[38.2, 44.1]$ and the Ensemble's $[39.2, 45.6]$, but this overlap by itself does \emph{not} license a ``statistically indistinguishable'' conclusion: two $95\%$ CIs can overlap while a paired test of the per-patient difference is significant. We therefore defer the FCOS-SGD-vs-RetinaNet question to the paired bootstraps in Section~\ref{sec:robustness} below, which directly test the per-patient difference. Likewise the Ensemble interval overlaps both Faster R-CNN and RetinaNet, so the right test of ``ensemble matches Faster R-CNN'' is again a paired bootstrap on the difference.
...
The Free-Response ROC curve is the standard localisation metric in medical-image detection: it plots sensitivity against false positives per image, with no per-image FP cap. Figure~\ref{fig:froc} shows the FROC curves and the column ``FROC CPM'' of Table~\ref{tab:robust_detection} reports the Competition Performance Metric (mean sensitivity at $\{0.125, 0.25, 0.5, 1, 2, 4, 8\}$ FP/image), the standardised summary statistic used in e.g.\ the LUNA16 nodule-detection challenge. We report CPM as a single point estimate per detector --- a CPM bootstrap CI would be the proper companion to the AP@0.5 CIs of Table~\ref{tab:robust_detection} but it is not implemented here, so the ordering between near-neighbours (Faster R-CNN $68.7$, Ensemble $68.1$, RetinaNet $66.5$, FCOS paper SGD $66.4$, FCOS Adam $62.7$) should be read as a coarse ranking rather than a significance test. With that caveat, CPM broadly tracks AP@0.5, FCOS Adam trails the rest by a few points, and the WBF Ensemble matches Faster R-CNN within $0.6$ CPM points despite trailing it by $0.9$ on AP@0.5 --- a result we attribute to WBF's coverage term suppressing singletons emitted by only one detector and thereby lowering false-positive density at every operating point. The same disclaimer applies to the ECE values and to the RSNA-percentile size-bucket APs of Table~\ref{tab:robust_detection}: those columns are bare point estimates, and the small-bucket numbers ($3.6$ for FCOS, $4.3$ for RetinaNet, $4.0$ for Faster R-CNN) are within plausible patient-resampling noise of each other given how few small-bucket boxes the validation split contains.
...
The patient-level F1 gap between detectors at the fixed $\tau = 0.3$ threshold (Table~\ref{tab:patient_class}) is dominated by score calibration. We quantify this with reliability diagrams (Figure~\ref{fig:calibration}) and the Expected Calibration Error (column ``ECE'' of Table~\ref{tab:robust_patient}). ECE is the patient-weighted mean absolute gap between predicted confidence and empirical positivity rate in 10 equal-width bins, with ECE $= 0$ indicating perfect calibration. As with CPM we report ECE as a point estimate per detector and do not attach a bootstrap CI; the two clusters that matter for the calibration argument --- RetinaNet ($10.7\%$) and Ensemble ($10.9\%$) on one side, FCOS Adam ($22.0\%$), FCOS paper SGD ($22.5\%$) and Faster R-CNN ($26.4\%$) on the other --- are separated by roughly $10$ ECE points, which is well outside the noise floor expected on $\sim 5{,}000$ validation patients, so the qualitative ordering is robust even without an explicit CI. Within each cluster, by contrast, the differences ($+0.2$ ECE between RetinaNet and Ensemble; $+0.5$ between FCOS Adam and FCOS SGD) should be treated as noise. RetinaNet and the Ensemble are therefore markedly better calibrated than the rest; the Ensemble inherits the calibration from averaging WBF over a focal-loss model and a centre-ness model, which acts as an implicit (uncalibrated) but effective post-hoc calibration step. This motivates the learnt aggregator below, which makes the calibration step explicit.
...
Three caveats apply uniformly to all five tests. (i) The five pairwise tests are not Holm/Bonferroni corrected; a Bonferroni-adjusted family-wise $\alpha = 0.05 / 5 = 0.01$ would push the Faster R-CNN vs.\ RetinaNet $p = 0.080$ further from significance and would leave only the FCOS (paper SGD) vs.\ Faster R-CNN test ($p < 0.001$) significant. (ii) ``Fails to reject the null at $5\%$'' does \emph{not} mean ``the two detectors are equivalent'': the bootstrap is run on a single training seed, so a real difference of $\sim 1$--$2$ AP that lies inside our run-to-run noise would be invisible. (iii) The point estimates themselves are read at each detector's best-validation EMA checkpoint and therefore carry the selection-on-val bias noted in Section~\ref{sec:results}.

With those caveats, the qualitative reading of the cached predictions is: \emph{Faster R-CNN is the only detector that strictly beats another on this split} --- $-3.07$ AP@0.5 vs.\ FCOS paper-SGD with $p < 0.001$ and a $95\%$ CI $[-4.84, -1.09]$ that does not contain zero (this also survives the Bonferroni $\alpha = 0.01$ cut). The remaining four pairwise differences (Faster R-CNN vs.\ RetinaNet, FCOS paper-SGD vs.\ RetinaNet, Ensemble vs.\ RetinaNet, Ensemble vs.\ Faster R-CNN) all fail to reject equivalence at $5\%$; the point estimates rank Faster R-CNN above Ensemble above RetinaNet above FCOS (paper SGD) above FCOS (Adam), but only the Faster R-CNN vs.\ FCOS-SGD and (transitively) the Faster R-CNN vs.\ FCOS-Adam separations are defensible. The Ensemble does \emph{not} beat single-model RetinaNet on AP@0.5 by a paired bootstrap ($+0.88$ AP, $p = 0.25$), so its previous AP@0.5 advantage was within patient-resampling noise. This complements the patient-level finding in Section~\ref{sec:robustness} that the Ensemble's fixed-threshold F1 advantage was a calibration artefact of the $\tau = 0.3$ cut.

\input{../results/paired_extras}
...
In our run, each gradient step is computed on a per-GPU batch of size $8$ over four H100s, processing $\sim 8{,}500$ training patients per epoch, so one epoch is $\sim 265$ steps. The EMA window $T_{\mathrm{eff}} = 1000$ steps therefore equals $\sim 3.8$ epochs, of the same order as our validation cadence of $4$ epochs. This is the precise reason why the EMA validation curves in Figure~\ref{fig:val_ap} are visibly smoother than what the raw iterate would produce: high-frequency noise components in the gradient estimate (per-batch noise, augmentation noise) decay with attenuation $1/(1 + (\omega T_{\mathrm{eff}})^2)^{1/2}$ in the magnitude response and are essentially eliminated above $\omega \gtrsim 1/T_{\mathrm{eff}}$.
...
\end{equation}
where $f$ is the density of the resampled statistic at the true quantile $q_p$. For our paired AP@0.5 differences with $\sigma \approx 1$ AP@0.5 point and a roughly Gaussian resample distribution, $1/f(q_{0.975}) \approx \sigma\sqrt{2\pi}\,\exp(z_{0.975}^2/2) \approx 6.3 \sigma$, so at $B = 200$:
\begin{equation}
\mathrm{SE}_{0.975}^{\mathrm{MC}} \approx 6.3 \cdot 1 \cdot \sqrt{0.025 \cdot 0.975 / 200} \approx 0.07\;\mathrm{AP\,points}.
\end{equation}
The Monte Carlo error on either endpoint of our paired CIs is therefore well below the $\sim 1$\,AP qualitative resolution we need, and re-running at $B = 1000$ would tighten the endpoints by a factor of $\sqrt 5 \approx 2.2\times$ to $\sim 0.03$ AP --- a refinement that would not change a single one of the qualitative conclusions in Table~\ref{tab:paired-extras}. The CI width itself ($\sim 4$ AP points on the paired differences) is set by the patient-resampling variance, \emph{not} by $B$, and the only way to tighten it is to enlarge the validation set or run additional training seeds.
...
The reference paper~\cite{wu2024pneumonia} argues that the anchor-free FCOS detector outperforms anchor-based alternatives on RSNA. Our experiments revise this claim in the opposite direction. On the full RSNA validation split, our anchor-free FCOS reaches $35.3\%$ (Adam) and $39.8\%$ (paper SGD); RetinaNet sits at $41.2\%$, Faster R-CNN at $42.9\%$. All four numbers exceed the paper's reported FCOS score ($28.5\%$). Under a shared Adam recipe the anchor-based detectors decisively win; under the paper's SGD recipe FCOS narrows the gap to RetinaNet but does \emph{not} demonstrably equalise the paradigms --- we only ran the SGD ablation on the FCOS side, so the comparison ``FCOS paper SGD vs.\ RetinaNet Adam'' is intrinsically mixed. We emphasise this asymmetry because the SGD ablation is the addition that rescues the FCOS narrative; the honest test of paradigm-vs-optimiser would also retrain RetinaNet and Faster R-CNN under SGD, which is left for future work. Three further observations contextualise the result.
...
The final four-model parallel training run completed in about $2.5$ wall-clock hours on four H100 80\,GB SXM GPUs, for a GPU-rental cost of approximately \$30 on RunPod's community cloud. Adding the cached-prediction inference pass and the post-hoc analyses, the entire pipeline from raw DICOM to compiled PDFs runs end-to-end in under three hours of GPU time. The public codebase (Section~\ref{sec:availability}) ships a deterministic patient-level $80/20$ split (seed $42$), incremental \texttt{history.json} writes after every validation (so an interrupted run is recoverable), the cached predictions and \texttt{all\_metrics\_v2.json} used to draw every figure in Section~\ref{sec:robustness}, and a single-command pipeline (\texttt{main.py --mode full}) for the training side. The split and the post-hoc analyses are bit-deterministic; the training run itself is \emph{not} --- TF32 + BF16 + asynchronous dataloader workers + nondeterministic CUDA atomics together break exact bit-level reproducibility, and a re-run of \texttt{main.py} on the same hardware will land within $\sim 1$ point of the cached numbers but not on top of them. Our statistical claims therefore rest on the cached predictions distributed with the report, not on the assumption that the run can be regenerated bit-for-bit.
...
Every reported number comes from a single training run per detector with a fixed seed of $42$, evaluated on the same patient-level $20\%$ validation split that was used for early-stopping / checkpoint selection, threshold fitting, the $5$-fold-CV learnt aggregator, and the bootstrap CIs. Section~\ref{sec:robustness} mitigates this in two limited ways: (i) the patient-level bootstrap quantifies how stable each detector's AP@0.5 is under \emph{patient resampling}, and (ii) the three-way split refits the Youden threshold on a calibration half so that the F1 number is not over-fit. Two important sources of variance remain uncovered: \emph{run-to-run variance} (initialisation, augmentation RNG, optimiser stochasticity), which is routinely $\pm 1$--$2$ AP for modern detectors and which a single seed cannot estimate; and the \emph{selection-on-val} bias from picking the best EMA checkpoint of each detector on the same set the headline AP is read on (with validation every $4$ epochs the max is over $\sim\!10$ noisy estimates, so the reported AP@0.5 of every detector in Table~\ref{tab:comparison} is upward-biased by an amount we do not quantify). The honest reading is therefore that AP@0.5 differences smaller than $\sim 2$ points should not be treated as resolved, and the pairwise comparisons of Section~\ref{sec:robustness} are reported without a Holm/Bonferroni correction for the four pairwise tests we run, so the $p = 0.080$ for Faster R-CNN vs.\ RetinaNet is best read as ``not significant'' rather than ``marginally significant''. A full $k$-seed retrain, a held-out test set carved out before any model selection, and a Bonferroni correction across pairwise tests remain future work.
...
Key findings:
\begin{enumerate}
    \item All detectors exceed the paper's reported FCOS AP@0.5 of $28.5\%$: our FCOS (Adam) reaches $35.3\%$, FCOS (paper SGD) $39.8\%$, RetinaNet $41.2\%$, and Faster R-CNN $42.9\%$. These point estimates carry two caveats that the patient-level bootstrap does not address: they are read at each detector's best-validation EMA checkpoint, which is a max over $\sim\!10$ noisy estimates and therefore upward-biased; and the bootstrap quantifies only test-sampling variance on a single training seed, so the $1$--$2$ AP run-to-run variance typical of detection is \emph{not} captured.
    \item \textbf{Under a shared Adam recipe the anchor-based detectors outperform the anchor-free FCOS on AP@0.5; under the paper's SGD recipe FCOS narrows the gap to RetinaNet, although our reading of the paired bootstrap (Section~\ref{sec:robustness}) does \emph{not} support a strong ``statistically indistinguishable'' claim.} We have run the SGD ablation only for FCOS, so we cannot rule out that RetinaNet or Faster R-CNN would also gain under SGD; the SGD result narrows the FCOS-vs-RetinaNet gap rather than equalising the paradigms.
    \item A Weighted Box Fusion ensemble of FCOS and RetinaNet reaches patient-level F1 $63.0\%$ at the fixed $\tau=0.3$ threshold, $+13.5$ points above the best single model \emph{at that arbitrary threshold}. This headline is largely an artefact of the calibration mismatch documented in Section~\ref{sec:robustness}: under a fair per-detector operating point (Youden threshold fitted on a calibration half, or the learnt aggregator), the ensemble's advantage shrinks to $\le 1$ F1 point over the best single detector (RetinaNet under the learnt aggregator and Faster R-CNN under the held-out Youden threshold). The fixed-threshold $\tau=0.3$ protocol is therefore the wrong place to make the ensemble case, even if it is the protocol the paper uses.
...
\noindent The single-run nature of the experiments is partly mitigated by the Section~\ref{sec:robustness} robustness checks --- patient-level bootstrap CIs on AP@0.5, a three-way split protocol that fits the Youden threshold on a held-out calibration half, RSNA-percentile size buckets, FROC/CPM, reliability diagrams with ECE, and a learnt 5-fold-CV patient-level aggregator --- but those bootstraps quantify only the variance of resampling validation patients on a single trained model, not the variance across training seeds. The remaining caveats --- a single training seed per detector, no held-out test set, no multiple-comparison correction across the pairwise tests, no per-site stratification and no external-dataset validation --- are stated in Section~\ref{sec:limitations}. End-to-end the pipeline runs in $\sim 2.5$ hours of wall-clock on $4\!\times\!$H100 for a GPU-rental cost of approximately \$30; that figure is a logistics note, not a contribution.
