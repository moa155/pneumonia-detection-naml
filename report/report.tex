...
Several techniques that are not specific to detection have become standard in modern recipes and make a measurable difference to our task: cosine learning-rate annealing~\cite{loshchilov2017sgdr}, BF16 or FP16 automatic mixed precision~\cite{micikevicius2018mixed}, and Exponential Moving Average (EMA) of the weights~\cite{polyak1992averaging, tarvainen2017mean}. We adopt all three.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{HorizontalFlip + VerticalFlip} ($p=0.5$ each): flip-based augmentation following the reference paper's $8\times$ expansion. Both are label-preserving geometric transforms (the boxes flip with the image) and are applied at training time only; evaluation uses horizontal-flip TTA on upright images.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{LR Schedule:} 2-epoch linear warm-up + cosine annealing to $10^{-6}$ for all four runs; the FCOS SGD ablation keeps this identical schedule and only swaps the optimiser (Adam $\rightarrow$ SGD, peak lr $5\times10^{-3}$)
...
\begin{equation}
    ...
    \end{cases},
\end{equation}
with $T = 40$, $T_w = 2$: SGDR~\cite{loshchilov2017sgdr} without restarts (analysed in Section~\ref{sec:numerical-cosine}). Our FCOS SGD ablation keeps this exact warm-up + cosine schedule and \emph{only} swaps the optimiser (Adam $\rightarrow$ SGD+momentum, peak lr $5\times10^{-3}$), so it is a clean optimiser ablation with everything else held constant. Wu et al.\ themselves use \emph{Adam} (lr $10^{-3}$), so our FCOS-Adam run is the optimiser-faithful reproduction and the SGD variant is our own controlled comparison.

\subsection{Exponential Moving Average at Validation Time}

The EMA weights (see Section~\ref{sec:background}) are used for all validation and final evaluation, while the raw optimiser iterates continue to drive the training loss. At save time, only the EMA weights are persisted as the ``best'' checkpoint, which is what we load in the final evaluation. We find EMA to be a substantial (though not always consistent) improvement: for RetinaNet and Faster R-CNN we observed $+0.5$ to $+1.5$ AP versus raw weights; for FCOS EMA smoothed the otherwise noisy validation trajectory.
...
The COCO ``medium'' bucket spans areas $[32^2, 96^2]$. On RSNA the median box area is $\approx 65{,}000$\,px$^2$ and only $0.9\%$ of the $9{,}555$ positive boxes fall in the COCO medium bucket, so the COCO $AP_M$ is computed over a handful of boxes and is essentially noise. We therefore report size-stratified AP on tertiles of the actual RSNA area distribution ($AP_S^{\text{RSNA}}, AP_M^{\text{RSNA}}, AP_L^{\text{RSNA}}$, Section~\ref{sec:robustness}), which are non-degenerate.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{detection_samples.png}
...
\noindent This calibration mismatch is precisely what WBF ensembling fixes: averaging FCOS's recall-greedy predictions with RetinaNet's precision-oriented ones moves the ensemble's optimal threshold to $\tau^{\star} = 0.299$, almost exactly the paper-defined cut of $0.3$, so the fixed-threshold F1 and the Youden-optimal F1 coincide. The ensemble's F1 of $63.0\%$ at $\tau = 0.3$ is $+9.8$ points over the best single model (Faster R-CNN, $53.2\%$), and the ensemble's precision of $56.1\%$ at the same threshold is $+19.7$ over Faster R-CNN ($36.4\%$). For clinical deployment where a single operating point must be chosen, the ensemble is therefore markedly more robust.
...
BF16 gave $\sim$1.8$\times$ training throughput over FP32 in preliminary runs and, by preserving FP32's exponent range, eliminated the occasional NaN/Inf we saw with FP16 on RetinaNet's focal-loss branch and the Faster R-CNN RPN (the wall-clock cost is reported in Section~\ref{sec:method}).
...
SGDR-style cosine with 2-epoch warm-up (Section~\ref{sec:numerical-cosine}) avoids the Adam cold-start that can destroy the pretrained backbone and replaces a hand-tuned step schedule with a smooth one; we use the same cosine schedule for all four runs, including the SGD ablation.
...
All five entries (FCOS Adam, FCOS SGD, RetinaNet, Faster R-CNN and the WBF ensemble) come from this single cached inference pass, with TTA and Soft-NMS enabled exactly as for the headline Table~\ref{tab:comparison}, so the AP@0.5 numbers below match that table within rounding.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{calibration.png}
...
\begin{enumerate}[nosep]
    ...
    \item The WBF ensemble of FCOS + RetinaNet reaches patient-level F1 $63.0\%$ at the fixed $\tau=0.3$ threshold, $+9.8$ over the best single model (Faster R-CNN). This is largely a calibration artefact (Section~\ref{sec:robustness}): under a fair operating point the ensemble no longer leads and is matched or overtaken by the best single model (RetinaNet reaches F1 $67.3\%$ with the learnt aggregator).
    \item Threshold-free ROC AUC lies in a tight $86.5$--$89.1\%$ band for all five entries, and the learnt aggregator lifts every single-detector F1 by $+10$ to $+22$ points into a $\sim 4$-point band, confirming that the large fixed-threshold F1 spread reflected score calibration rather than detection quality.
\end{enumerate}
